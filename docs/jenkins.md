...
> HyperEnv for Jenkins: Besides the open-soure template we are also offering a commercial product for running [Jenkins on AWS](https://hyperenv.com/jenkins/). HyperEnv for Jenkins launches agents faster, as it uses a Jenkins plugin instead of an custom agent. Also HyperEnv for Jenkins comes with built-in support for OpenID Connect with AWS Cognito or your own identity provider.
